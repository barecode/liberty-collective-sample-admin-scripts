...
def parse(args) {
	...
	parser.setUsage('groovy stopServer.groovy [--help] --serverHost=serverHost --serverUsrDir=serverUserDir --serverName=serverName --truststore=truststoreName --truststorePassword=truststorePassword --host=hostname --port=port --user=username --password=password [--options=options]')
	parser.setHeader('\nUsed to stop a server specified by the serverHost, serverUserDir, and serverName. \nThe following options are available:\n') 
	parser.setFooter('\nExample: groovy stopServer.groovy --serverName=member1 --serverHost=localhost --serverUsrdir=/wlp/usr --truststore=/wlp/usr/servers/<servername>/resources/security/trust.jks --truststorePassword=secret --host=localhost --port=9443 --user=Administrator --password=secret2 --options=\"--clean --include=someValue\"')
...
stopServerOption=options.options
if (!stopServerOption)
	stopServerOption=""
result=command.stop(options.serverHost,options.serverUsrDir,options.serverName,stopServerOption)

returnCode=Integer.parseInt(result.get("returnCode"))
if (returnCode==0)
	println "Server stopped successfully"
else if (returnCode==1)
	println "Server alrady stopped"
else {
	println "Server did not stop: return code = " + returnCode

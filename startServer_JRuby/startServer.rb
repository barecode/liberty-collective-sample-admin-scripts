...
# startServer.rb
#
# Starts a server with the given host name, user directory, and server name.  
...
#
# ex. jython startServer.py serverHost /usr/wlp/usr serverName --truststore=/home/user/key.jks --trustStorePassword=secret --host=host.com --port=9443 --user=Administrator --password=secret2

require "wlp_mbean_args_parser"
require "wlp_server_command"

class StartServerArgsParser < MBeanArgsParser
	OPTIONS="--options"
	def initialize
		@options=[OPTIONS]
	end
	...
	def printUsage()
		puts ""
		print "Usage: jruby startServer.rb [--debug] serverHost serverUserDir serverName"
		print getBaseUsage
		puts " [--options=\"options list\"]"
		puts ""
		puts "For HELP: jruby startServer.rb --help"
	end

	def printHelp
		printUsage()
		puts ""
		puts "Used to start a server specified by the serverHost, serverUserDir, and serverName."
		puts ""
		printBaseHelp()
		puts ""
    		puts "Example: jython startServer.py serverHost serverUserDir serverName --truststore=/home/user/key.jks --trustStorePassword=secret --host=host.com --port=9443 --user=Administrator --password=secret2 --options=\"--clean --include=someValue\""
	end
...
class StartServer
	...
	def execute()
		...
		options = @optional_value_args[StartServerArgsParser::OPTIONS]
		...
		result = serverCommand.start(host,userdir,server,options)
		return result
	end
end

# main begins here
parser = StartServerArgsParser.new()
if(parser.parse(ARGV) == false)
	exit()
end

startServer = StartServer.new(parser)
result = startServer.execute()
if parser.debug?
...
if returnCode == 0
    ...
    puts "Server did not start: return code = " + returnCode.to_s
    exit(returnCode)
end

...
#
# transferConfigToServer.py
#
# Provides operations to transfer server.xml configuration to a Liberty collective member
...
# --serverUsrdir= The user directory where the collective member is installed.
...
#
# ex. jython transferConfigToServer.py /home/server1/server.xml --serverName=member1 --serverHost=localhost --serverUsrdir=/wlp/usr --truststore=/wlp/usr/servers/<servername>/resources/security/trust.jks --truststorePassword=secret --host=localhost --port=9443 --user=Administrator --password=secret2 
#
...
# Subclass of command line arguments for transferConfigToServer
class TransferAppToServerArgs(MBeanArgs):
  ...
  def printUsage(self):
    print "Usage: jython transferConfigToServer.py path/to/server.xml " + self.getUsage()

  # Print required options
  def printRequiredHelp(self):
    print SERVER_NAME + "= The name of the server to transfer the server.xml to "
    print
    print SERVER_HOST + "= The host name where the collective member is installed"
    print
    print SERVER_USRDIR + "= The user directory where the collective member is installed"
    print
  ...
  def printHelp(self):
    self.printUsage()
    print ""
    print "Used to tranfer a server.xml to a member server manged by a collective"
    print "controller.  The collective controller is also referred to as the"
    print "\"routing server\" in some messages."
    print
    print "This action will push the server.xml file to the member server"
    print "Ensure that the remote member directory is "
    ...
    print
    print "Example: jython transferConfigToServer.py /home/server1/server.xml --serverName=member1 --serverHost=localhost --serverUsrdir=/wlp/usr --truststore=/wlp/usr/servers/<servername>/resources/security/trust.jks --truststorePassword=secret --host=localhost --port=9443 --user=Administrator --password=secret2 "
...
if __name__ == '__main__':
  ...
  if (argParser.parse(sys.argv) == True):
    try:
      ...
      print "Pushing the server.xml to server " +argParser[SERVER_HOST]+","+argParser[SERVER_USRDIR]+","+argParser[SERVER_NAME]
      serverConfig.pushConfig(configPath)
         
    except java.lang.Throwable, t:
      print "An exception was caught while processing the transferConfigToServer command"
      if (arguments.DEBUG not in argParser):
        print t.toString()
      else:
        t.printStackTrace()
    except Exception, e:
      print "A python exception was caught while processing the transferConfigToServer command"
      if (arguments.DEBUG not in argParser):
        print e
      else:
